...
see an illustration in Figure~\ref{fig:domain}. % Yes, not the same orientation, get over it!
...
\section{TEST CASES}
\label{sec:tests}
In this section we will show the NURBS conversion of more complex objects.
...
so a comparison using default domains is more informative.
All images in this section were created by a commercially available CAD system,
Rhinoceros 3D~\cite{Rhino3D}.

The model in Figure~\ref{fig:cagd86}, defined by quintic $G^1$ frames,
has three 3-sided, two 4-sided, a 5-sided and a 6-sided surface.
...
in Fig.~\ref{fig:cagd86-6-cg} is, while somewhat deviating, still managable.
\begin{figure}[!ht]
  {
    \hfill
    \begin{subfigure}{.3\textwidth}
...
We find similar results in Figure~\ref{fig:dolphin}, where a dolphin model
is shown, also defined by quintic $G^1$ frames. The two 6-sided S-patches are
not evaluated correctly due to the erratic control points.
...
\begin{figure}[!ht]
...

  {
    ...
    \begin{subfigure}{.45\textwidth}
      ...
      \caption{6-sided Charrot--Gregory patch (head)}
    ...
    \begin{subfigure}{.45\textwidth}
      ...
      \caption{6-sided Charrot--Gregory patch (body)}
      \label{fig:dolphin-5-cg}
    \end{subfigure}
    \hfill
  }
  \caption{Dolphin model.}
  \label{fig:dolphin}
\end{figure}
