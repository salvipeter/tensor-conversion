...
\end{equation}
the B\'ezier coefficients are computed as $C^\top MC$, where $C=\{c_{ij}\}$ is
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The control points are then computed by calculating the coefficients for the numerator and
denominator of Eq.~(\ref{eq:spatch}) separately, and assigning them to homogeneous coordinates
(with the latter acting as the weight).
