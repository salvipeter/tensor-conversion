...
naturally involve modeling multi-sided (i.e., non-four-sided) surfaces, as well.  The
...
rational polynomial of two parametric variables. Then it can be directly converted into
...
described, and a general discussion will conclude the paper.
...
Kato's patch is singular at the corner vertices; the S-patch is singular on the circle that goes
through the intersections of the lines containing the domain edges; and the Charrot--Gregory patch
...
\begin{table}
  ...
  \begin{tabular}{c|c|c|c|c}
    ...
    3 & \multicolumn{2}{c|}{$d$ [$d+3$]} & \cellcolor{light-gray}$3d+5$ & $d+3$ \\ \hline
    5 & \cellcolor{light-light-gray}$3d$ [$3d+9$] & $\approx 3d$ & \cellcolor{light-gray}$5d+9$ & $5d+6$ \\ \hline
    6 & \cellcolor{light-light-gray}$4d$ [$4d+12$] & $3d$ & \cellcolor{light-gray}$6d+11$ & $6d+8$ \\ \hline
    7+ & \cellcolor{light-gray}$(n-2)d$ [$(n-2)d+3n-6$] & $\qquad$N/A$\qquad$ & \cellcolor{light-gray}$nd+2n-1$ & \cellcolor{light-gray}$nd+2n-4$ \\ \hline
  \end{tabular}
  \caption{Rational polynomial degrees of the converted surfaces for different number of sides,
    ...
    the singularity issue. Note that triangular S-patches and Warren patches are both B\'ezier
    triangles. For S-patches, the number in brackets shows the degree when the surface is
    generated by a degree-$d$ $G^1$ frame.}
  \label{tab:degrees}
\end{table}

Table~\ref{tab:degrees} summarizes the degrees of all four representations. While Warren's patch
is excellent in this respect, the use of base points somewhat limits its usability in CAD systems.
Kato's surface always has singularities, and its degree is fairly high, but it is the only
construction where $G^2$ continuity is easy to achieve. We found that while the
Charrot--Gregory patch has a slightly higher degree than the S-patch, it has a much lower
computational cost in its multi-sided form, and does not have control net quality issues for 5-
and 6-sided configurations.

\vspace{10pt}
\noindent\underline{Conclusion:}\vspace{0.2em}\newline
We have reviewed four $n$-sided surface representations, investigated their exact
conversion into trimmed tensor product form,
and presented an efficient conversion method using implicit line equations.
We have also discussed possible issues, and proposed a solution to the singularity problem.
